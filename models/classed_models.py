...
# 2. Balanced Evaluation & Reporting Engine
def run_rf_and_evaluate_2025(X_hist, y_hist, X_2025, y_2025, model_name):
    # Stratified split ensures the validation split maintains training class balance
    ...
    rf = RandomForestClassifier(
        ...
        n_jobs=-1,
        oob_score=True,
        class_weight="balanced",  # Optimizes split rules internally for imbalanced entries
    )
    ...
    # --- EVALUATION 2: Balanced Unseen 2025 Test Dataset ---
    print("\n" + "." * 65)
    print(" RUNNING MODEL ON BALANCED UNSEEN 2025 DATASET")
    print("." * 65)
    
    if X_2025.empty:
        print("No valid 2025 records available matching this model's target criteria.")
    else:
        # Re-attach target vector momentarily to handle uniform downsampling
        eval_df_2025 = X_2025.copy()
        eval_df_2025['TARGET_LABEL'] = y_2025
        
        # Enforce downsampling to balance the test evaluation classes evenly
        min_class_size = eval_df_2025['TARGET_LABEL'].value_counts().min()
        
        if min_class_size == 0 or len(eval_df_2025['TARGET_LABEL'].unique()) < 2:
            print("Skipping balanced evaluation: Unseen 2025 set completely lacks one of the classes.")
        else:
            balanced_2025 = eval_df_2025.groupby('TARGET_LABEL').sample(n=min_class_size, random_state=42)
            
            X_2025_balanced = balanced_2025[feature_cols]
            y_2025_balanced = balanced_2025['TARGET_LABEL']
            
            print(f"-> Balanced 2025 evaluation sample size: {len(X_2025_balanced)} rows ({min_class_size} per class)")
            
            y_pred_2025 = rf.predict(X_2025_balanced)
            report_2025 = classification_report(y_2025_balanced, y_pred_2025, output_dict=True)
            ...
            print("\n2025 BALANCED CONFUSION MATRIX")
            cm_2025 = confusion_matrix(y_2025_balanced, y_pred_2025, labels=labels)
...
run_rf_and_evaluate_2025(X1_hist, y1_hist, X1_2025, y1_2025, "Any Impact vs. No Impacts (Binary)")
