...
# =========================================================================
# 3. REGISTER FILE MAPS BY ACCUMULATION TYPE
# =========================================================================
...
# =========================================================================
# 5. SUBPRODUCT MULTI-FILE TIME-SERIES LOOP (BOUND TO APRIL WITH 3HR LAG)
# =========================================================================
...
# Color Table mapping: 0 is completely hidden by GIS, 1 is bright red
color_table = {
    0: (0, 0, 0, 0),        # 0 = No Impact -> Transparent
    1: (255, 0, 0, 255)     # 1 = Impact -> Red
}

print("\nStarting execution loop for April with 3-hour product lag...")

for i in range(5, len(all_timestamps)):
    current_ts = all_timestamps[i]
    
    if not current_ts.startswith("202504"):
        print(f"\n[+] Encountered non-April file ({current_ts}). Stopping script processing execution.")
        break
    ...
    if not all([file_01h_0, file_01h_1, file_01h_2, file_03h, file_24h, file_48h]):
        print(f"[-] SKIPPING interval {ts_now}: Missing paired product step.")
        continue
    ...
    try:
        ...
        for out_name, (model, class_labels) in models_to_run.items():
            # Crucial Fix: Initialize the array as 0 (Transparent/No-Data template)
            ...
            if len(valid_pixel_mask) > 0:
                ...
                # Assign values (0 for no impact, 1 for flood impact) inside your mask boundary
            ...
            profile = {
                ...
                'nodata': 0  # Crucial Fix: Forces GIS to make all 0 values completely transparent
            }
        ...
        for k in old_keys: del cache_01h[k]
...
print("\nTime-series script sequence finished processing!")
